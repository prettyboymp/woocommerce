...
class StockNotificationsMetaDataStore extends CustomMetaDataStore {
	...
	protected function get_cache_group() {
		return 'stock_notifications_meta';
	}
	 ...
	 *
	 * @param  \WC_Data  $notification WC_Data object.
	 * @param  \stdClass $meta (containing at least ->id).
	 *
	 * @return bool
	 */
	public function delete_meta( &$notification, $meta ): bool {
		$successful = parent::delete_meta( $notification, $meta );
		if ( $successful ) {
			$this->clear_cached_data( array( $notification->get_id() ) );
		}
	...
	public function add_meta( &$notification, $meta ) {
		$insert_id = parent::add_meta( $notification, $meta );
		if ( false !== $insert_id ) {
			$this->clear_cached_data( array( $notification->get_id() ) );
		}
	...
	public function update_meta( &$notification, $meta ): bool {
		$is_successful = parent::update_meta( $notification, $meta );
		if ( $is_successful ) {
			$this->clear_cached_data( array( $notification->get_id() ) );
		}
	...
	/**
	 * Return notification meta data for multiple IDs. Results are cached.
	 *
	 * @param array $notification_ids List of notification IDs.
	 *
	 * @return \stdClass[][] An array, keyed by the object IDs, containing arrays of raw meta data for each object.
	 */
	public function get_meta_data_for_object_ids( array $notification_ids ): array {
		if ( ! $this->is_cache_enabled() ) {
			return parent::get_meta_data_for_object_ids( $notification_ids );
		}

		$meta_data  = $this->get_meta_data_for_object_ids_from_cache( $notification_ids );
		$notification_ids = array_diff( $notification_ids, array_keys( $meta_data ) );

		if ( empty( $notification_ids ) ) {
			return $meta_data;
		}

		$db_meta_data = parent::get_meta_data_for_object_ids( $notification_ids );
	 ...
	 *
	 * @param int[] $notification_ids List of object IDs.
	 *
	 * @return \stdClass[][] An array, keyed by the object IDs, containing arrays of raw meta data for each object.
	 */
	private function get_meta_data_for_object_ids_from_cache( array $notification_ids ): array {
		$cache_engine = wc_get_container()->get( WPCacheEngine::class );
		$meta_data    = $cache_engine->get_cached_objects( $notification_ids, $this->get_cache_group() );
	 ...
	 *
	 * @param array $notification_ids The object_ids to delete cache for.
	 *
	 ...
	 */
	public function clear_cached_data( array $notification_ids ): array {
		if ( ! $this->is_cache_enabled() ) {
			return array_fill_keys( $notification_ids, true );
		}

		$cache_engine  = wc_get_container()->get( WPCacheEngine::class );
		$return_values = array();
		foreach ( $notification_ids as $notification_id ) {
			$return_values[ $notification_id ] = $cache_engine->delete_cached_object( $notification_id, $this->get_cache_group() );
		}
