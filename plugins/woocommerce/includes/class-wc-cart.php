	...
	*/
	...
	public function add_to_cart( $product_id = 0, $quantity = 1, $variation_id = 0, $variation = array(), $cart_item_data = array() ) {
		try {
			...
			// If cart_item_key exists, update quantity.
			$existing_cart_id = $this->find_product_in_cart( $cart_item['key'] );
			if ( $existing_cart_id ) {
				$this->set_quantity(
					$existing_cart_id,
					$quantity + $this->cart_contents[ $existing_cart_id ]['quantity'],
					true
				);
			...
			// Add item to cart.
			$this->cart_contents[ $cart_item['key'] ] = $cart_item;

			do_action( 'woocommerce_add_to_cart', $cart_item['key'], $product_id, $quantity, $variation_id, $variation, $cart_item_data );
