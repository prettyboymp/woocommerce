	...
	public function execute_tool( $tool ) {
		...
		switch ( $tool ) {
			...
			case 'clear_sessions':
				...
				$result = absint( $wpdb->query( "DELETE FROM {$wpdb->usermeta} WHERE meta_key='_woocommerce_persistent_cart_" . get_current_blog_id() . "';" ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				wp_cache_flush();
