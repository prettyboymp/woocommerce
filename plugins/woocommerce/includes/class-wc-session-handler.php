...
class WC_Session_Handler extends WC_Session {
	...
	public function init_session_cookie() {
		...
		if ( ! $cookie ) {
			// This ID will be regenerated on every request until its stored to a cookie, typically only after adding an item to the cart.
			$this->_customer_id = $this->generate_customer_id();
			$this->_data        = $this->get_session_data();
			// Update session expiration locally.
			$this->set_session_expiration();
			return;
		}

		// Customer ID will be an MD5 hash if this is a guest session, or user ID if logged in.
		...
		$is_expiring = $this->is_session_expiring();
		$is_valid    = $this->is_session_cookie_valid();
		...
		// If the user logs in and the session is not already a user session, migrate the session data to the user session.
